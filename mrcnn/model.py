...
def detection_targets_graph(proposals, gt_class_ids, gt_boxes, gt_masks, gt_attribute_ids, config):
    ...
    gt_masks = tf.gather(gt_masks, tf.where(non_zeros)[:, 0], axis=2,
                         name="trim_gt_masks")
    # fan
    gt_attribute_ids = tf.boolean_mask(gt_attribute_ids, non_zeros,
                                   name="trim_gt_attribute_ids")
    ...
    # fan
    gt_attribute_ids = tf.gather(gt_attribute_ids, non_crowd_ix)
    ...
    # fan
    roi_gt_attribute_ids = tf.gather(gt_attribute_ids, roi_gt_box_assignment)
    ...
    # fan
    roi_gt_attribute_ids = tf.pad(roi_gt_attribute_ids, [(0, N + P), (0, 0)])
    return rois, roi_gt_class_ids, deltas, masks, roi_gt_attribute_ids
...
class DetectionTargetLayer(KE.Layer):
    ...
    def call(self, inputs):
        ...
        gt_masks = inputs[3]
        # fan
        gt_attribute_ids = inputs[4]

        # Slice the batch and run a graph for each slice
        # TODO: Rename target_bbox to target_deltas for clarity
        # fan
        names = ["rois", "target_class_ids", "target_bbox", "target_mask", "gt_attribute_ids"]
        outputs = utils.batch_slice(
            [proposals, gt_class_ids, gt_boxes, gt_masks, gt_attribute_ids],
...
def fpn_classifier_graph(rois, feature_maps, image_meta,
                         pool_size, num_classes, num_attributes, train_bn=True,
                         fc_layers_size=1024):
...
def load_image_gt(dataset, config, image_id, augment=False, augmentation=None,
    ...
    mask, class_ids, attribute_ids = dataset.load_mask(image_id)
    ...
    # fan attribute_ids:[num_instances, NUM_ATTRIBUTES]
    attribute_ids = attribute_ids[_idx,:]
    ...
    return image, image_meta, class_ids, bbox, mask, attribute_ids
    ...
    while True:
        try:
            ...
            if dataset.image_info[image_id]['source'] in no_augmentation_sources:
                image, image_meta, gt_class_ids, gt_boxes, gt_masks, gt_attribute_ids = \
            ...
            if b == 0:
                ...
                batch_gt_attribute_ids = np.zeros(
                    (batch_size, config.MAX_GT_INSTANCES, config.NUM_ATTRIBUTES), dtype=np.int32)
            ...
            if gt_boxes.shape[0] > config.MAX_GT_INSTANCES:
                ...
                gt_attribute_ids = gt_attribute_ids[ids]
            ...
            if b >= batch_size:
                ...
                inputs = [batch_images, batch_image_meta, batch_rpn_match, batch_rpn_bbox,
                          batch_gt_class_ids, batch_gt_boxes, batch_gt_masks, batch_gt_attribute_ids]
                outputs = []
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            if config.USE_MINI_MASK:
                ...
                input_gt_masks = KL.Input(
                    ...
                    name="input_gt_masks", dtype=bool)
            # 4. GT Attributes (zero padded) fan
            # [batch, attributes=294]
            input_gt_attribute_ids = KL.Input(
                shape=[None, config.NUM_ATTRIBUTES], name="input_gt_attribute_ids", dtype=tf.int32)
        ...
        if mode == "training":
            ...
            inputs = [input_image, input_image_meta,
                      input_rpn_match, input_rpn_bbox, input_gt_class_ids, input_gt_boxes, input_gt_masks, input_gt_attribute_ids]
            if not config.USE_RPN_ROIS:
                inputs.append(input_rois)
            outputs = [rpn_class_logits, rpn_class, rpn_bbox,
                       mrcnn_class_logits, mrcnn_class, mrcnn_bbox, mrcnn_mask,
                       rpn_rois, output_rois,
                       rpn_class_loss, rpn_bbox_loss, class_loss, bbox_loss, mask_loss, attribute_loss]
    ...
    def compile(self, learning_rate, momentum):
        ...
        loss_names = [
            ...
            "mrcnn_class_loss", "mrcnn_bbox_loss", "mrcnn_mask_loss", "mrcnn_attribute_loss"]
